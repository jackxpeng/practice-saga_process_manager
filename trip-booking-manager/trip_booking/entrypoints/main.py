...
from trip_booking.domain.domain import ProcessState
...
@app.post("/trips")
def create_trip(req: TripRequest, service: TripApplicationService = Depends(get_service)):
...
def approve_trip(booking_id: str, req: ApprovalRequest, service: TripApplicationService = Depends(get_service)):
    try:
